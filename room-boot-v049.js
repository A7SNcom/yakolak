...
console.info('[Yakolak] ROOM BOOT v060 FAST CACHED ROOM LOADED');

const BUILD='60';
const B={floorY:-430,topY:1180,halfW:2400,backZ:-2400,frontZ:2400,camPad:260};

if(!globalThis.__yakolakRoomPatchV060){
  globalThis.__yakolakRoomPatchV060=true;
  ...
  function material(color){return new THREE.MeshStandardMaterial({color,roughness:1,metalness:0,side:THREE.FrontSide})}
  function panel(scene,name,w,h,x,y,z,rx,ry,rz,mat){
    ...
    mesh.receiveShadow=false; mesh.renderOrder=-1000;
    add0.call(scene,mesh); return mesh;
  }
  function line(scene,name,a,b,opacity=.24){
    ...
    mesh.name=name; mesh.renderOrder=-900; add0.call(scene,mesh); return mesh;
  }
  function room(scene){
    ...
    const w=B.halfW*2,d=B.frontZ-B.backZ,h=B.topY-B.floorY,my=B.floorY+h/2,cz=0;
    const floorMat=material(0xe9e4da),ceilMat=material(0xfaf8f1),wallMat=material(0xf4f0e8),frontMat=material(0xf4f0e8);
    frontMat.transparent=true; frontMat.opacity=.08; frontMat.depthWrite=false;
    ...
    line(scene,'yakolak-room-back-floor-line',[-B.halfW,B.floorY,B.backZ],[B.halfW,B.floorY,B.backZ],.42);
    line(scene,'yakolak-room-front-floor-line',[-B.halfW,B.floorY,B.frontZ],[B.halfW,B.floorY,B.frontZ],.08);
    line(scene,'yakolak-room-left-back-corner',[-B.halfW,B.floorY,B.backZ],[-B.halfW,B.topY,B.backZ],.28);
    line(scene,'yakolak-room-right-back-corner',[B.halfW,B.floorY,B.backZ],[B.halfW,B.topY,B.backZ],.28);
    line(scene,'yakolak-room-left-floor-line',[-B.halfW,B.floorY,B.backZ],[-B.halfW,B.floorY,B.frontZ],.18);
    line(scene,'yakolak-room-right-floor-line',[B.halfW,B.floorY,B.backZ],[B.halfW,B.floorY,B.frontZ],.18);
    console.info('[Yakolak] FAST ROOM ACTIVE',{floorY:B.floorY,topY:B.topY,width:w,depth:d});
  }
  function clampCamera(scene,camera){
    if(!scene||!scene.__yakolakRoomBounds||!camera?.position)return;
  ...
  THREE.TextureLoader.prototype.load=function(url,onLoad,onProgress,onError){
    ...
    if(s.includes('Asset%201big.svg')||s.includes('Asset 1big.svg')){
      console.info('[Yakolak] SVG background skipped; fast room active');
