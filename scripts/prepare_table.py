#!/usr/bin/env python3
"""Create a real-time 3D tabletop from the approved YAKOLAK table.svg footprint."""
...
def triangulate(points: list[Point]) -> list[Triangle]:
    # Ear clipping on a simple counter-clockwise polygon.
    ...
    while len(remaining) > 3:
        ...
        if guard > len(points) * len(points):
            raise RuntimeError("Could not triangulate approved table footprint")
        clipped = False
        for offset, current in enumerate(remaining):
            ...
            break
        if not clipped:
            raise RuntimeError("Approved table footprint contains an invalid polygon")
...
def write_obj(points: list[Point]) -> None:
    ...
    with OUTPUT.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("# Approved YAKOLAK table footprint extruded from table.svg\n")
        handle.write("o approved_table\n")
        for x, z in points:
            handle.write(f"v {x:.6f} 0.000000 {z:.6f}\n")
        for x, z in points:
            handle.write(f"v {x:.6f} {THICKNESS:.6f} {z:.6f}\n")

        # The triangulator is CCW in X/Z. Reverse top winding for +Y normals.
    ...
    print(
        f"table.obj: {len(points)} approved footprint points, "
        f"{len(top_triangles) * 2 + len(points) * 2} triangles, span={TARGET_SPAN}, thickness={THICKNESS}"
    )
