...
requireTokens(css,['.content-card','.task-row','.drag-handle','.status-in_progress','.task-detail','.comment-form','.task-feed-tabs','.comment.worker','.preview-controls','@media(max-width:760px)','[hidden]{display:none!important}'],'task styles');
requireTokens(ui,[
...
requireTokens(api,[
  ...
  "action==='task_work_add'","status==='in_progress'","status='planned'","actor==='manager'&&status==='done'",'president_approval_required','RASHED_PORTAL_KEY','x-yakolak-manager-key','YAKOLAK_WORKER_KEY','x-yakolak-worker-key','cleanAttachments'
],'persistent task API');
...
if(blueprint.schemaVersion!==1||blueprint.revision!==7||!Array.isArray(blueprint.nodes)||!Array.isArray(blueprint.edges))fail('invalid blueprint revision');
...
if(!minimalNode||minimalNode.revision!==7||minimalNode.taskId!=='YAK-010')fail('delegated-work amendment missing');
for(const edge of blueprint.edges)if(!blueprintIds.has(edge.from)||!blueprintIds.has(edge.to))fail(`invalid edge ${edge.id}`);

if(ledger.schemaVersion!==1||ledger.revision!==7||!Array.isArray(ledger.initiatives)||!Array.isArray(ledger.tasks))fail('invalid ledger revision');
const initiativeIds=uniqueIds(ledger.initiatives,'initiatives'),taskIds=uniqueIds(ledger.tasks,'tasks');
if(ledger.portfolio?.activeInitiativeId!=='initiative-minimal-card-workspace'||!taskIds.has('YAK-010-01'))fail('YAK-010 is not active');
...
if(status.blueprint?.canonicalRevision!==7||status.blueprint?.lastReconciledPresidentRevision!==7)fail('President status revision mismatch');
if(manifest.version!==5||manifest.taskFeeds?.join(',')!=='president,work'||manifest.views?.join(',')!=='cards,ordered-tasks'||manifest.taskStatuses?.join(',')!=='planned,in_progress,review,done'||manifest.managerAuthentication!=='RASHED_PORTAL_KEY'||manifest.workerAuthentication!=='YAKOLAK_WORKER_KEY')fail('task runtime manifest mismatch');
