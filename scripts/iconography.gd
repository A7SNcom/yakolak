...
const CLOSE_ICON: Texture2D = preload("res://assets/icons/lucide/x.svg")
const MENU_ICON: Texture2D = preload("res://assets/icons/lucide/ellipsis.svg")
...
func _apply_iconography() -> void:
	...
	if close_ok and menu_ok:
		...
		push_error("YAKOLAK_ICON_AUDIT_FAILED close=%s menu=%s" % [str(close_ok), str(menu_ok)])
