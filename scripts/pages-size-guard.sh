...
# GitHub platform ceilings (documented externally): Pages published site 1 GiB,
# regular Git object 100 MiB. YAKOLAK intentionally stays far below both.
PAGES_PLATFORM_BYTES=$((1024 * 1024 * 1024))
GIT_PLATFORM_FILE_BYTES=$((100 * 1024 * 1024))

# Internal hard budgets. Do not raise these to make a deployment pass. Crossing
# one requires the tested immutable external-asset path in PAGES_SIZE_GUARDRAILS.md.
...
# GitHub's repository API reports repository size in KiB. In Actions this is a
# hard repo-growth guard; local runs simply skip it when no token is available.
