...
try {
  ...
  await page.mouse.click(...Object.values(await screenPoint({ type: 'color', value: 'right' })));
  await page.waitForFunction(() => globalThis.__yakolakGame?.state?.setupStep === 'bots', null, { timeout: 8_000 });
  await page.mouse.click(...Object.values(await screenPoint({ type: 'bots', value: 3 })));
  ...
  await page.waitForFunction(() => globalThis.__yakolakGame?.pieces?.some(piece => piece.mesh.userData.traySelected), null, { timeout: 8_000 });
  ...
  await page.waitForFunction(target => globalThis.__yakolakGame?.state?.board?.[target.id]?.[target.size] === target.color, zonePoint, { timeout: 10_000 });
