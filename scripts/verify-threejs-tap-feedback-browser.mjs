...
try {
  ...
  const result = await page.evaluate(async () => {
    ...
    const controller = tapModule.createTapClickConfirmationController({
      ...
      // This latency probe exercises size selection/feedback only. Board-pick data are
      // intentionally omitted so metric availability cannot depend on unrelated assets.
      worldLayout: {},
      approvedContract: {},
    });
    ...
    for (let index = 0; index < 10; index += 1) {
      trigger.dispatchEvent(new PointerEvent('pointerup', {
        bubbles: true, cancelable: true, pointerId: index + 1, pointerType: 'touch', isPrimary: true,
      }));
    }
    ...
    for (let index = 0; index < 60; index += 1) {
      ...
      trigger.dispatchEvent(new PointerEvent('pointerup', {
        bubbles: true, cancelable: true, pointerId: index + 20, pointerType: 'touch', isPrimary: true,
      }));
      const processingEndedAt = performance.now();
      const expected = before + 1;
      synchronous.push(feedbackSequence === expected && marker.dataset.sequence === String(expected));
      processingMs.push(processingEndedAt - startedAt);
      ...
      visibleMs.push(performance.now() - startedAt);
    }
