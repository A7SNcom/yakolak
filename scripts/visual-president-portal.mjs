...
const validReview = {
  ...
  prUrl: 'https://github.com/A7SNcom/yakolak/pull/43',
...
async function verify(viewport, name) {
  ...
  await page.getByText('مسار تطوير ياكلك').waitFor();
  await page.getByText('العقود والحالة الحتمية للدخول').waitFor();
  if (await page.locator('[data-blueprint-node]').count() < 5) throw new Error(`${name}: project map did not render enough nodes`);
  ...
  await page.getByText('العقود والحالة الحتمية للدخول').waitFor();
  if (!String(await page.locator('[data-task-id="YAK-004-01"] .president-progress-label').textContent()).includes('مراجعة')) throw new Error(`${name}: task progress is not visible`);
  await page.screenshot({path: path.join(artifacts, `${name}-tasks.png`), fullPage: true});

  await page.getByRole('button', {name: /^السجل/}).click();
  await page.getByText('إنتاج PR #41').waitFor();
