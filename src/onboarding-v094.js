...
export function startOnboardingV094(){
  ...
  function conditionMet(wait){const g=game(),s=state();if(wait==='setup-color')return s.setupStep==='bots';if(wait==='setup-bots')return s.configured;if(wait==='tray-open')return g?.pieces?.some(p=>p.mesh?.userData?.inTray);if(wait==='piece-selected')return g?.pieces?.some(p=>p.mesh?.userData?.traySelected);if(wait==='human-move')return boardCount(s.humanColor)>baseline.human;if(wait==='bot-move')return Object.values(s.board||{}).reduce((n,c)=>n+Object.values(c||{}).filter(v=>v&&v!==s.humanColor).length,0)>baseline.bot;return false}
