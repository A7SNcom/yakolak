const response = await fetch('./src/app-game-v112.js?v=D1-review-runtime-wrapper', { cache: 'no-store' });
if (!response.ok) throw new Error(`D1 runtime wrapper load failed: ${response.status}`);
...
replaceExact(
  ...
  "const response=await fetch('./src/app-game-v085.js?v=D1-review-runtime-source',{cache:'no-store'});",
  'D1 source marker'
);
...
const releasePatch = [
  ...
  '  "globalThis.__yakolakGame={state:gameState,pieces,boardZones,camera,renderer,gameGroup,setupGroup,gameHighlightGroup,THREE,controls,meshes,render,clearHighlights,showWinHighlight,syncZoneMarkers,setResponsiveOverview,renderSetup3D,closePieceTray,updateTurnGlow,syncActiveReadinessBases};",',
  "  'expose D1 controls and native scene bridge'",
  ');'
].join('\n');

replaceExact(
  "source+='\\n//# sourceURL=yakolak-v112-action-tutorial-runtime.js\\n';",
  releasePatch + "\nsource+='\\n//# sourceURL=yakolak-developer-d1-runtime.js\\n';",
  'inject D1 runtime policies'
);
replaceExact(
  "globalThis.__yakolakV112={build:112,base:110,tutorial:'short-skippable-action-led'};",
  "globalThis.__yakolakV117={build:117,base:116,change:'reuse-native-setup-tray-and-table-online'};globalThis.__yakolakV120={build:120,base:119,change:'mobile-only-board-separation'};globalThis.__yakolakDeveloperD1Runtime={build:'D1',controls:true};",
  'D1 runtime marker'
);
...
try {
  ...
  await import('./online-client-v114.js?v=D1-review-runtime-client');
} finally {
