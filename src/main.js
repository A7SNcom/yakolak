...
function init() {
  ...
  camera.position.set(95, 155, 185);

  controls = new OrbitControls(camera, renderer.domElement);
  controls.enableDamping = true;
  controls.target.set(calibration.origin.x, calibration.origin.y, calibration.origin.z);
...
function addLights() {
  const hemi = new THREE.HemisphereLight(0xffffff, 0x7b6c5c, 1.35);
  scene.add(hemi);

  const key = new THREE.DirectionalLight(0xffffff, 2.25);
  key.position.set(70, 180, 95);
  key.castShadow = true;
  key.shadow.mapSize.set(2048, 2048);
  key.shadow.camera.near = 1;
  key.shadow.camera.far = 450;
  scene.add(key);
}

function addWorld() {
  const radius = Math.max(Math.abs(calibration.stepX), Math.abs(calibration.stepZ), calibration.trayRadius) * 1.65;
  const floor = new THREE.Mesh(
    new THREE.CircleGeometry(radius, 128),
    new THREE.MeshStandardMaterial({ color: 0xe6daca, roughness: 0.92, metalness: 0.02 })
  );
  floor.rotation.x = -Math.PI / 2;
  floor.position.y = calibration.origin.y - 0.35;
...
function buildBoard() {
  ...
  const boardWidth = Math.abs(calibration.stepX) * 3.35;
  const boardDepth = Math.abs(calibration.stepZ) * 3.35;
  const base = new THREE.Mesh(
    new THREE.BoxGeometry(boardWidth, 0.18, boardDepth),
    new THREE.MeshStandardMaterial({ color: 0xc7a16d, roughness: 0.78, metalness: 0.02 })
  );
  base.position.set(calibration.origin.x, calibration.origin.y - 0.09, calibration.origin.z);
  ...
  for (let i = 0; i < 9; i += 1) {
    ...
    const radius = Math.min(Math.abs(calibration.stepX), Math.abs(calibration.stepZ)) * 0.25;
    const marker = new THREE.Mesh(
      new THREE.TorusGeometry(radius, 0.45, 12, 96),
      new THREE.MeshStandardMaterial({ color: 0x6f4e2f, roughness: 0.85 })
    );
    marker.rotation.x = Math.PI / 2;
    marker.position.set(pos.x, calibration.origin.y + 0.12, pos.z);
    ...
    dot.position.set(pos.x, calibration.origin.y + 0.16, pos.z);
    slotGroup.add(dot);
  }
}
...
function onCalibrationInput() {
  ...
  const step = Math.max(0.5, readNumber(inputs.gridStep, Math.abs(calibration.stepX)));
  calibration.stepX = step;
  calibration.stepZ = -step;
...
function syncInputs() {
  ...
  inputs.gridStep.value = fixed(Math.abs(calibration.stepX));
...
function updateCalibrationOutput() {
  const cell1 = getSlotPosition(0);
  const cell9 = getSlotPosition(8);
  const exportAlignment = {
    version: YAKLAK_ALIGNMENT.version,
    note: YAKLAK_ALIGNMENT.note,
    cell1: { x: fixedNumber(cell1.x), y: fixedNumber(cell1.z), z: fixedNumber(calibration.origin.y) },
    cell9: { x: fixedNumber(cell9.x), y: fixedNumber(cell9.z), z: fixedNumber(calibration.origin.y) },
    cellHalf: YAKLAK_ALIGNMENT.cellHalf,
    snapZOffset: fixedNumber(calibration.dropHeight),
    derived: {
      stepX: fixedNumber(calibration.stepX),
      stepY: fixedNumber(calibration.stepZ),
      stepZ: 0
    }
  };
  output.value = 'const YAKLAK_ALIGNMENT = ' + JSON.stringify(exportAlignment, null, 2) + ';';
}
...
function onPointerDown(event) {
  ...
  if (activePiece.userData.slotIndex !== null) removePieceFromSlot(activePiece);

  activePiece.userData.slotIndex = null;
  activePiece.position.y = getPieceY() + 12;
...
function findNearestSlot(position) {
  ...
  const snapRange = Math.min(Math.abs(calibration.stepX), Math.abs(calibration.stepZ)) * 0.52;
  return best && best.distance <= snapRange ? best : null;
}
...
function layoutTrayPieces() {
  ...
  ordered.forEach((piece) => {
    ...
    const sideOffset = (count - 1) * 10;
    const innerOffset = (sizeRank - 1) * 16;
    ...
    const home = new THREE.Vector3(
      calibration.origin.x + Math.cos(sideAngle) * radial + tangent.x,
      getPieceY(),
      calibration.origin.z + Math.sin(sideAngle) * radial + tangent.z
    );
...
function getSlotPosition(index) {
  ...
  return new THREE.Vector3(
    calibration.origin.x + (col - 1) * calibration.stepX,
    calibration.origin.y,
    calibration.origin.z + (row - 1) * calibration.stepZ
  );
}
