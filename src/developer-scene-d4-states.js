...
Object.assign(document.body.dataset,{preview:preview?'1':'0',developerEntityKind:entityKind,developerEntity:entityId,developerVariant:variant});
if(sceneId)document.body.dataset.developerScene=sceneId;
if(elementId)document.body.dataset.developerElement=elementId;
if(status)status.textContent=`D4 · ${entityId} · ${variant}`;
...
function markReady(details={}){Object.assign(document.body.dataset,{sceneReady:'true',...details});globalThis.__yakolakDeveloperD4State={build:'D4',entityKind,entityId,variant,...details};parent.postMessage({type:'yakolak-developer-scene-ready',entityKind,entityId,scene:sceneId,element:elementId,variant,build:'D4',details},'*')}
...
function selectVisualPiece(game,size='l',color='front'){
  const piece=game.pieces.find(item=>item.type===size&&item.dir===color&&!item.placed);if(!piece)return null;
  piece.mesh.position.y+=18;piece.mesh.scale.multiplyScalar(1.06);return piece;
}
function configureGameplay(game){const count=variant==='two-players'?2:4;setupPlay(game,{count});game.caption?.(`بداية اللعب · ${count===2?'لاعبان':'أربعة لاعبين'}`);return{mode:'game-state',composition:'gameplay-ready',players:String(count)}}
function configureLegalMoves(game){setupPlay(game,{count:4});const size={large:'l',medium:'m',small:'s'}[variant]||params.get('size')||'l';const piece=selectVisualPiece(game,size,'front');game.syncZoneMarkers?.(true);game.caption?.(`اختر خانة متاحة للقطعة ${size==='l'?'الكبيرة':size==='m'?'المتوسطة':'الصغيرة'}.`);render(game);return{mode:'game-state',composition:'legal-moves',size,selected:String(Boolean(piece))}}
function configureTurn(game){const color=params.get('color')||variant||'front';setupPlay(game,{count:4,color});game.state.currentIndex=Math.max(0,game.state.players.indexOf(color));game.syncScoreHud?.();game.updateTurnGlow?.();game.caption?.(`الدور الآن: ${COLOR_LABEL[color]||color}`);game.startTurnTimer?.();render(game);return{mode:'game-state',composition:'turn-state',activeColor:color}}
function showTutorial(game,mode=variant){setupPlay(game,{count:2});if(mode==='guided'){game.state.firstMoveGuide=true;game.syncZoneMarkers?.(true);selectVisualPiece(game,'l','front');game.caption?.('افتح طقمك، اختر أي حجم، ثم ضع القطعة في خانة متاحة.');overlay('تعلّم داخل أول حركة','المؤقت متوقف في أول دور فقط، وتبقى اللوحة قابلة للتجربة.',['فهمت']);render(game);return{mode:'tutorial',composition:'guided-first-turn'}}
...
function configureWinner(game,preset=variant){setupPlay(game,{count:2});game.previewWinnerHighlightPreset?.(preset);if(!game.previewWinnerHighlightPreset)game.debugTriggerWin('same-size','front');render(game);return{mode:'win-state',composition:'winner-highlight',preset}}
function configureResult(game,type=variant){setupPlay(game,{count:2});const win=game.debugTriggerWin(type,'front');overlay('فاز الأخضر بالجولة',type==='graded'?'خط متدرج: صغير ثم متوسط ثم كبير.':type==='cell'?'اكتملت الأحجام الثلاثة في خانة واحدة.':'ثلاث قطع من الحجم نفسه في خط واحد.',['الجولة التالية']);render(game);return{mode:'result-state',composition:'round-result',winType:type,triggered:String(Boolean(win))}}
function onlineCard(mode){
  const copy=mode==='room-code'?'أدخل رمز الغرفة المكوّن من الحروف والأرقام للانضمام.':mode==='waiting'?'الغرفة جاهزة؛ ينتظر النظام اكتمال المشاركين واستعدادهم.':'أنشئ غرفة جديدة أو انضم إلى غرفة موجودة.';
  const actions=mode==='room-code'?['انضمام','رجوع']:mode==='waiting'?['نسخ الرمز','مغادرة']:['إنشاء غرفة','لدي رمز'];
  return overlay(mode==='waiting'?'غرفة الانتظار':mode==='room-code'?'الانضمام إلى غرفة':'اللعب أونلاين',copy,actions);
}
function configureOnline(game,mode=variant){setupPlay(game,{count:2,showHud:false});const native=showDom('yakolakOnlineEntry');if(native){native.classList.add('open');native.dataset.developerVariant=mode}else onlineCard(mode);return{mode:'online-state',composition:mode,nativeEntry:String(Boolean(native))}}
function configureZoneElement(game){setupPlay(game,{count:2,showHud:false});if(variant==='occupied'){const piece=game.pieces.find(item=>item.dir==='front'&&item.type==='l');if(piece){piece.placed=true;piece.zoneIndex=4;piece.slotSize='l';game.state.board[4].l='front';const zone=game.boardZones[4];piece.mesh.position.set(zone.px,zone.py,zone.pz)}}game.syncZoneMarkers?.(true);game.camera.position.set(250,300,250);game.controls.target.set(0,0,0);game.controls.update();render(game);return{mode:'ui-element',composition:'zone-marker',state:variant}}
function configureScoreElement(game){setupPlay(game,{count:2});const count={one:1,three:3,five:5}[variant]||1;for(let index=0;index<count;index++)game.debugScorePoint?.('front');game.caption?.(`${count} ${count===1?'نقطة':'نقاط'} للأخضر`);render(game);return{mode:'ui-element',composition:'score-marker',count:String(count)}}
function configureHudElement(game){const count=variant==='two-players'?2:4;setupPlay(game,{count});game.caption?.('دورك: اختر قطعة ثم خانة متاحة.');return{mode:'ui-element',composition:'game-hud',players:String(count)}}
...
async function run(){
  ...
  if(sceneId==='gameplay-ready')details=configureGameplay(game);
  else if(sceneId==='legal-moves')details=configureLegalMoves(game);
  else if(sceneId==='turn-state')details=configureTurn(game);
  else if(sceneId==='tutorial-first-move')details=showTutorial(game);
  else if(sceneId==='winner-highlight')details=configureWinner(game,params.get('preset')||variant);
  else if(sceneId==='round-result')details=configureResult(game,params.get('win')||variant);
  else if(sceneId==='online-entry')details=configureOnline(game,variant);
  else if(elementId==='zone-marker')details=configureZoneElement(game);
  else if(elementId==='score-marker')details=configureScoreElement(game);
  else if(elementId==='game-hud')details=configureHudElement(game);
  else if(elementId==='tutorial-dialog')details=showTutorial(game,'prompt');
  else if(elementId==='online-panel')details=configureOnline(game,variant);
  else if(elementId==='winner-glow')details=configureWinner(game,variant);
  else throw new Error(`Unknown D4 state ${entityId}`);
  ...
  if(['winner-highlight','winner-glow'].includes(entityId))replayTimer=setInterval(()=>configureWinner(game,variant),4200);
}
...
run().catch(error=>{console.error('[Yakolak] D4 state preview failed',error);if(status)status.textContent='D4 · ERROR';document.body.dataset.sceneError=String(error?.message||error);parent.postMessage({type:'yakolak-developer-scene-error',entityKind,entityId,variant,error:String(error?.message||error)},'*')});
