...
const ROOM_LIMIT={
  minX:-ROOM_CFG.halfW+90,
  maxX: ROOM_CFG.halfW-90,
  minY: ROOM_CFG.floorY+80,
  maxY: ROOM_CFG.topY-70,
  minZ: ROOM_CFG.backZ+90,
  maxZ: ROOM_CFG.frontZ-90
};
...
function keepInsideRoom(){
  ...
  controls.target.x=clamp(controls.target.x,-420,420);
  controls.target.y=clamp(controls.target.y,ROOM_CFG.floorY+80,ROOM_CFG.topY-170);
  controls.target.z=clamp(controls.target.z,-420,420);
}
...
scene.add(new THREE.HemisphereLight(0xffffff,0xd7e0e5,1.2));
const key=new THREE.DirectionalLight(0xffffff,1.45);key.position.set(-450,700,500);scene.add(key);
const rim=new THREE.DirectionalLight(0xffffff,.32);rim.position.set(420,360,-620);scene.add(rim);
const fill=new THREE.PointLight(0xffffff,.55,2400);fill.position.set(480,360,520);scene.add(fill);
...
function addRoom(){
  ...
  const edgeMat=new THREE.LineBasicMaterial({color:0x9eacb5,transparent:true,opacity:.86});
  const gridMat=new THREE.LineBasicMaterial({color:0xc9d3da,transparent:true,opacity:.38});
  ...
  const t=12,base=18;
  box('base-back',w,base,t,0,floorY+base/2,backZ+t/2);
  box('base-front',w,base,t,0,floorY+base/2,frontZ-t/2);
  box('base-left',t,base,d,-halfW+t/2,floorY+base/2,0);
  box('base-right',t,base,d,halfW-t/2,floorY+base/2,0);
  box('ceiling-back',w,t,t,0,topY-t/2,backZ+t/2);
  box('ceiling-front',w,t,t,0,topY-t/2,frontZ-t/2);
  box('ceiling-left',t,t,d,-halfW+t/2,topY-t/2,0);
  box('ceiling-right',t,t,d,halfW-t/2,topY-t/2,0);
  ...
  [[xL,floorY,zB,xL,topY,zB],[xR,floorY,zB,xR,topY,zB],[xL,floorY,zF,xL,topY,zF],[xR,floorY,zF,xR,topY,zF],
   [xL,floorY,zB,xR,floorY,zB],[xL,floorY,zF,xR,floorY,zF],[xL,floorY,zB,xL,floorY,zF],[xR,floorY,zB,xR,floorY,zF],
   [xL,topY,zB,xR,topY,zB],[xL,topY,zF,xR,topY,zF],[xL,topY,zB,xL,topY,zF],[xR,topY,zB,xR,topY,zF]]
   .forEach(a=>line([[a[0],a[1],a[2]],[a[3],a[4],a[5]]]));

  for(let x=-1200;x<=1200;x+=300)line([[x,floorY+2,zB],[x,floorY+2,zF]],gridMat);
  for(let z=-1200;z<=1200;z+=300)line([[xL,floorY+3,z],[xR,floorY+3,z]],gridMat);
  return group;
}
addRoom();
...
function table(){const g=new THREE.Group(),topMat=makeMat({color:'#c79a64',roughness:.82,metalness:0}),sideMat=makeMat({color:'#7a4b27',roughness:.86,metalness:0});const top=set(new THREE.Mesh(new THREE.BoxGeometry(470,24,360),topMat));top.position.y=ROOM_CFG.floorY+272;g.add(top);const legGeo=new THREE.BoxGeometry(28,260,28);[[-190,-130],[190,-130],[-190,130],[190,130]].forEach(([x,z])=>{const leg=set(new THREE.Mesh(legGeo,sideMat));leg.position.set(x,ROOM_CFG.floorY+130,z);g.add(leg)});scene.add(g);return g}
