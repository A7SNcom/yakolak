...
function pieces(geos){const types=['l','m','s'];bases.forEach(([dir,bx,bz,ang])=>[-1,0,1].forEach(side=>types.forEach(type=>{const r=rad(ang),x=bx+Math.cos(r)*48*side,z=bz+Math.sin(r)*48*side;const mesh=set(new THREE.Mesh(geos[type],mats[dir]));scene.add(mesh);tr(mesh,{px:x,py:2,pz:z,rx:-90,ry:0,rz:0})})))}
...
async function boot(){try{status('تحميل سريع...');const [g9,g3,gl,gm,gs]=await Promise.all([loadSTL('9',center),loadSTL('3',center),loadSTL('l',bottom),loadSTL('m',bottom),loadSTL('s',bottom)]);const objects=[];const board=set(new THREE.Mesh(g9,baseMat));scene.add(board);tr(board,A['9']);objects.push(board);['right','left','front','back'].forEach(d=>{const m=set(new THREE.Mesh(g3,baseMat));scene.add(m);tr(m,A['3-'+d]);objects.push(m)});pieces({l:gl,m:gm,s:gs});objects.push(...scene.children.filter(o=>o.isMesh));objects.push(table());fitCamera(objects);render();requestAnimationFrame(()=>requestAnimationFrame(reveal));marble();status('جاهز');log('FAST STATIC READY v'+BUILD)}catch(e){fatal(e)}}
