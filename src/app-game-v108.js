const response=await fetch('./src/app-game-v085.js?v=108-direct-source',{cache:'no-store'});
if(!response.ok)throw new Error(`v108 source load failed: ${response.status}`);
...
globalThis.__yakolakPerformance={enabled:true,profile:'v108-viewer-light',mobileHighQuality:MOBILE_HIGH_QUALITY,get pixelRatio(){return renderer?.getPixelRatio?.()||performancePixelRatio()}};
`,
...
replaceRegex(
  ...
  dragLightHandles=[];
  const oldViewer=camera.getObjectByName('yakolak-v108-viewer-light');
  const oldViewerTarget=camera.getObjectByName('yakolak-v108-viewer-target');
  ...
  const viewer=new THREE.DirectionalLight(0xfffaf4,.34);
  viewer.name='yakolak-v108-viewer-light';
  viewer.target.name='yakolak-v108-viewer-target';
  viewer.position.set(0,90,160);
  viewer.target.position.set(0,-20,-700);
  camera.add(viewer,viewer.target);
}
function applyCalibration()`,
  'camera-facing viewer light rig'
);
...
replaceExact("lightRig.name='yakolak-calibration-light-rig';","lightRig.name='yakolak-v108-viewer-light-rig';",'light rig name');
source+='\n//# sourceURL=yakolak-v108-viewer-light-runtime.js\n';

globalThis.__yakolakV108={build:108,style:'viewer-light',lights:3,room:'unlit-uniform'};
