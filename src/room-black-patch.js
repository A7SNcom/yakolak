...
const blackParts = new Set(['room-floor','room-ceiling']);
const whiteParts = new Set(['room-back-wall','room-left-wall','room-right-wall','room-front-wall']);
const oldAdd = THREE.Object3D.prototype.add;
function applyColor(obj){
  if(!obj || !obj.isMesh) return;
  const black = blackParts.has(obj.name);
  const white = whiteParts.has(obj.name);
  if(!black && !white) return;
  const materials = Array.isArray(obj.material) ? obj.material : [obj.material];
  for (const mat of materials){
    if(!mat) continue;
    if(mat.color && mat.color.set) mat.color.set(black ? 0x000000 : 0xfafcfd);
