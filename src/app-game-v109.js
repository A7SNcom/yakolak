const response=await fetch('./src/app-game-v085.js?v=109-direct-source',{cache:'no-store'});
if(!response.ok)throw new Error(`v109 source load failed: ${response.status}`);
...
replaceExact("const BUILD='97';","const BUILD='109';",'build number');
replaceExact(
...
globalThis.__yakolakPerformance={enabled:true,profile:'v109-clear-playfield',mobileHighQuality:MOBILE_HIGH_QUALITY,get pixelRatio(){return renderer?.getPixelRatio?.()||performancePixelRatio()}};
`,
...
replaceRegex(
  ...
  game:{
    board:{color:'#343b44',roughness:.52,metalness:0,emissive:'#111820',emissiveIntensity:.14},
...
replaceRegex(
  ...
  `const makeMat=p=>new THREE.MeshStandardMaterial(p);
const baseMat=makeMat({color:'#343b44',roughness:.52,metalness:0,emissive:'#111820',emissiveIntensity:.14});
...
replaceRegex(
  ...
  dragLightHandles=[];
  const oldViewer=camera.getObjectByName('yakolak-v109-viewer-light');
  const oldViewerTarget=camera.getObjectByName('yakolak-v109-viewer-target');
  ...
  viewer.name='yakolak-v109-viewer-light';
  viewer.target.name='yakolak-v109-viewer-target';
  viewer.position.set(0,130,220);
  viewer.target.position.set(0,-40,-850);
  camera.add(viewer,viewer.target);
}
function applyCalibration()`,
  'clear three-light playfield rig'
);
...
replaceExact("lightRig.name='yakolak-calibration-light-rig';","lightRig.name='yakolak-v109-clear-playfield-rig';",'light rig name');
source+='\n//# sourceURL=yakolak-v109-clear-playfield-runtime.js\n';

globalThis.__yakolakV109={build:109,style:'clear-playfield',lights:3,room:'unlit-uniform'};
