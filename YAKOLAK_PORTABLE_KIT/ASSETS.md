...
| `assets/layout/world-layout.json` | machine-readable board, bases, room, and cameras |
| `assets/layout/intro-scatter.csv` | exact 36 starting transforms for the unboxing intro |
| `assets/manifest.json` | machine-readable asset list |
...
- Use `intro-scatter.csv` directly when exact cross-language intro matching is required; do not rely only on a random seed.
- The official SVGs embed a light/white fill. Preserve their paths, but tint them to dark ink such as `#3f3f3f` when displayed on the light wall.
