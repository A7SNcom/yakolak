...
const ITEM_KINDS=new Set(['thread','request','entity']);
let client;
let tableReady;
...
async function ensureTable(db){tableReady||=db.execute(`CREATE TABLE IF NOT EXISTS ${TABLE} (item_key TEXT PRIMARY KEY,item_kind TEXT NOT NULL,before_url TEXT NOT NULL DEFAULT '',after_url TEXT NOT NULL DEFAULT '',updated_at TEXT NOT NULL)`);await tableReady}
...
function cleanKey(value){const key=cleanText(value,220);if(!/^(thread|request|entity):[a-zA-Z0-9:_-]{1,190}$/.test(key))throw new Error('invalid_key');return key}
function cleanKind(value){const kind=cleanText(value,20);if(!ITEM_KINDS.has(kind))throw new Error('invalid_kind');return kind}
function cleanUrl(value){const url=cleanText(value,2048);if(!url)return'';let parsed;try{parsed=new URL(url,'https://yakolak.local/')}catch{throw new Error('invalid_url')}if(!['http:','https:'].includes(parsed.protocol))throw new Error('invalid_url');return url}
...
async function save(db,body){const itemKey=cleanKey(body.itemKey),itemKind=cleanKind(body.itemKind),beforeUrl=cleanUrl(body.beforeUrl),afterUrl=cleanUrl(body.afterUrl),updatedAt=new Date().toISOString();await db.execute({sql:`INSERT INTO ${TABLE}(item_key,item_kind,before_url,after_url,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(item_key) DO UPDATE SET item_kind=excluded.item_kind,before_url=excluded.before_url,after_url=excluded.after_url,updated_at=excluded.updated_at`,args:[itemKey,itemKind,beforeUrl,afterUrl,updatedAt]});return{itemKey,itemKind,beforeUrl,afterUrl,updatedAt}}
...
export default async function handler(req,res){
  ...
  }catch(error){const status=statusFor(error);if(status>=500)console.error('[Yakolak] Developer comparison store failed',error);json(res,status,{ok:false,error:status>=500?'comparison_store_error':error.message})}
}
