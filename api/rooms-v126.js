...
function publicRoom(row, state = null) {
  ...
  return { code: String(row.room_code), version: Number(row.version), ...parsed };
}
...
function lobbyPreview(row) {
  ...
  return {
    ...
    name: normalizeRoomName(state.name) || 'غرفة ياكلك',
    status: state.status,
...
async function listRooms(db) {
  const result = await db.execute({
    sql: `
      SELECT room_code, state_json, version, updated_at
      FROM ${TABLE}
      WHERE status = 'waiting' AND expires_at > ?
      ORDER BY updated_at DESC
      LIMIT ?
    `,
    args: [new Date().toISOString(), ROOM_LIST_LIMIT]
  });
  return (result.rows || []).flatMap(row => {
    try {
      const room = lobbyPreview(row);
      return room.players.length < room.targetPlayers ? [room] : [];
    } catch {
      return [];
    }
  });
...
async function updateRoom(db, row, state, expectedVersion, auth = null) {
  ...
  const result = await db.execute({
    sql: `
      UPDATE ${TABLE}
      SET auth_json = ?, state_json = ?, status = ?, version = version + 1,
          updated_at = ?, expires_at = ?
      WHERE room_code = ? AND version = ?
    `,
    args: [
      JSON.stringify(auth || authEntries(row)),
...
async function createRoom(db, roomName, color, targetPlayers, targetRounds) {
  const name = normalizeRoomName(roomName);
  if (!validRoomName(name)) throw new Error('invalid_room_name');
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    const code = roomCode();
    const state = { ...createOnlineState(color, targetPlayers, targetRounds), name };
...
async function joinRoom(db, code, color) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    const result = await db.execute({
      ...
      `,
      args: [
        JSON.stringify(auth), JSON.stringify(next), next.status, now,
        isoAfter(ROOM_TTL_MS), code, Number(row.version)
      ]
    });
...
function statusFor(error) {
  ...
  if ([
    'not_your_turn', 'room_full', 'version_conflict', 'color_taken',
    'room_not_playing', 'round_not_finished', 'occupied_slot', 'no_piece_remaining'
  ].includes(code)) return 409;
  if ([
    'invalid_color', 'invalid_player_count', 'invalid_round_count', 'invalid_room_name',
    'invalid_move', 'invalid_room_code', 'invalid_payload', 'invalid_action'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'list') {
      json(res, 200, { ok: true, rooms: await listRooms(db) });
      return;
    }
    if (action === 'preview') {
    ...
    if (action === 'create') {
      const result = await createRoom(
        ...
        body.roomName,
        String(body.color || ''),
        Number(body.targetPlayers),
        Number(body.targetRounds)
      );
    ...
    let next;
    if (action === 'move') {
      next = applyOnlineMove(state, seat, { zone: Number(body.zone), size: String(body.size || '') });
    ...
    if (status >= 500) {
      console.error('[Yakolak] v126 room request failed', {
        action: String(req.body?.action || ''),
        message: String(error?.message || error),
        stack: error?.stack
      });
    }
    json(res, status, { ok: false, error: status >= 500 ? 'online_server_error' : error.message });
  }
}
