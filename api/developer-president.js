...
const BLUEPRINT_TABLE='yakolak_president_blueprints';
const EVENT_TABLE='yakolak_president_events';
const BLUEPRINT_KEY='development-main';
const MAX_BODY_BYTES=280_000;
const MAX_BLUEPRINT_BYTES=220_000;
...
const NODE_TYPES=new Set(['goal','scene','task','decision','risk','evidence']);
const NODE_STATUSES=new Set(['idea','documented','ready','in_progress','review','completed','blocked','cancelled']);
...
async function ensureTables(db){
  tablesReady||=(async()=>{
    ...
    await db.execute(`CREATE TABLE IF NOT EXISTS ${BLUEPRINT_TABLE} (board_key TEXT PRIMARY KEY,board_json TEXT NOT NULL,base_revision INTEGER NOT NULL DEFAULT 0,version INTEGER NOT NULL DEFAULT 1,updated_by TEXT NOT NULL,created_at TEXT NOT NULL,updated_at TEXT NOT NULL)`);
...
const blueprintFromRow=row=>row?({boardKey:String(row.board_key),board:parseJson(row.board_json,{nodes:[],edges:[]}),baseRevision:Number(row.base_revision||0),version:Number(row.version||0),updatedBy:String(row.updated_by||''),createdAt:String(row.created_at||''),updatedAt:String(row.updated_at||'')}):null;
async function recordEvent(db,action,payload){
  const createdAt=new Date().toISOString();
  const result=await db.execute({sql:`INSERT INTO ${EVENT_TABLE}(action,payload_json,created_at) VALUES(?,?,?) RETURNING id`,args:[action,JSON.stringify(payload||{}),createdAt]});
  const eventId=Number(result.rows?.[0]?.id||0);
  console.info('[Yakolak President]',JSON.stringify({eventId,action,...payload,createdAt}));
  return{eventId,createdAt};
}
async function readBlueprint(db){
  const result=await db.execute({sql:`SELECT * FROM ${BLUEPRINT_TABLE} WHERE board_key=? LIMIT 1`,args:[BLUEPRINT_KEY]});
  return blueprintFromRow(result.rows?.[0]);
}
async function readAll(db){
  const [directives,messages,decisions,blueprint]=await Promise.all([
  ...
  return{directives:(directives.rows||[]).map(directiveFromRow),messages:(messages.rows||[]).map(messageFromRow),decisions:(decisions.rows||[]).map(decisionFromRow),blueprint};
}
async function activitySummary(db,afterEventId=0){
  const after=Math.max(0,Number(afterEventId)||0);
  const [latest,events]=await Promise.all([
    db.execute(`SELECT id,created_at FROM ${EVENT_TABLE} ORDER BY id DESC LIMIT 1`),
    db.execute({sql:`SELECT id,action,payload_json,created_at FROM ${EVENT_TABLE} WHERE id>? ORDER BY id ASC LIMIT 100`,args:[after]})
  ]);
  const rows=events.rows||[],latestRow=latest.rows?.[0];
  return{
    latestEventId:Number(latestRow?.id||0),
    latestActivityAt:String(latestRow?.created_at||''),
    afterEventId:after,
    newCount:rows.length,
    hasNewPresidentInput:rows.length>0,
    events:rows.map(row=>({id:Number(row.id),action:String(row.action),payload:parseJson(row.payload_json,{}),createdAt:String(row.created_at)}))
  };
}
...
function cleanBlueprint(value){
  const source=value&&typeof value==='object'&&!Array.isArray(value)?value:{};
  const rawNodes=Array.isArray(source.nodes)?source.nodes:[],rawEdges=Array.isArray(source.edges)?source.edges:[];
  if(rawNodes.length>100||rawEdges.length>250)throw new Error('blueprint_too_large');
  const nodes=rawNodes.map(raw=>{
    const id=cleanId(raw.id),type=cleanText(raw.type,24),status=cleanText(raw.status,32),title=cleanText(raw.title,180);
    if(!NODE_TYPES.has(type)||!NODE_STATUSES.has(status)||!title)throw new Error('invalid_blueprint');
    const x=Math.max(0,Math.min(4000,Number(raw.x)||0)),y=Math.max(0,Math.min(3000,Number(raw.y)||0));
    return{id,type,status,title,body:cleanText(raw.body,6000),owner:cleanText(raw.owner,80),taskId:cleanText(raw.taskId,120),evidenceUrl:cleanText(raw.evidenceUrl,1000),x,y,revision:Math.max(1,Math.floor(Number(raw.revision)||1))};
  });
  const nodeIds=new Set(nodes.map(node=>node.id));
  const edges=rawEdges.map(raw=>{
    const id=cleanId(raw.id),from=cleanId(raw.from),to=cleanId(raw.to);
    if(from===to||!nodeIds.has(from)||!nodeIds.has(to))throw new Error('invalid_blueprint');
    return{id,from,to,label:cleanText(raw.label,120)};
  });
  const board={schemaVersion:1,title:cleanText(source.title||'مسار تطوير ياكلك',180),nodes,edges};
  if(Buffer.byteLength(JSON.stringify(board),'utf8')>MAX_BLUEPRINT_BYTES)throw new Error('blueprint_too_large');
  return board;
}
async function saveBlueprint(db,body){
  const board=cleanBlueprint(body.board),expectedVersion=Math.max(0,Math.floor(Number(body.expectedVersion)||0)),baseRevision=Math.max(0,Math.floor(Number(body.baseRevision)||0)),now=new Date().toISOString();
  const current=await readBlueprint(db);
  let result;
  if(!current){
    if(expectedVersion!==0)throw new Error('blueprint_conflict');
    result=await db.execute({sql:`INSERT INTO ${BLUEPRINT_TABLE}(board_key,board_json,base_revision,version,updated_by,created_at,updated_at) VALUES(?,?,?,?,?,?,?) RETURNING *`,args:[BLUEPRINT_KEY,JSON.stringify(board),baseRevision,1,'president',now,now]});
  }else{
    if(current.version!==expectedVersion)throw new Error('blueprint_conflict');
    result=await db.execute({sql:`UPDATE ${BLUEPRINT_TABLE} SET board_json=?,base_revision=?,version=version+1,updated_by='president',updated_at=? WHERE board_key=? AND version=? RETURNING *`,args:[JSON.stringify(board),baseRevision,now,BLUEPRINT_KEY,expectedVersion]});
    if(!result.rows?.[0])throw new Error('blueprint_conflict');
  }
  const blueprint=blueprintFromRow(result.rows[0]);
  await recordEvent(db,'blueprint_save',{boardKey:BLUEPRINT_KEY,version:blueprint.version,baseRevision,nodeCount:board.nodes.length,edgeCount:board.edges.length});
  return blueprint;
}
function statusFor(error){
  if(['payload_too_large','blueprint_too_large'].includes(error?.message))return 413;
  if(['invalid_id','invalid_directive','invalid_message','invalid_decision','decision_requires_comment','invalid_blueprint','invalid_payload'].includes(error?.message))return 400;
  if(error?.message==='directive_not_found')return 404;
  if(['blueprint_conflict'].includes(error?.message)||String(error?.message||'').includes('UNIQUE constraint failed'))return 409;
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='GET'){
      if(String(req.query?.summary||'')==='1'){json(res,200,{ok:true,channelVersion:2,...await activitySummary(db,req.query?.after)});return}
      json(res,200,{ok:true,channelVersion:2,activity:await activitySummary(db,0),...await readAll(db)});return;
    }
    ...
    else if(action==='blueprint_save')json(res,200,{ok:true,blueprint:await saveBlueprint(db,body)});
