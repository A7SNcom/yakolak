...
async function createRoom(db, color, targetPlayers, targetRounds) {
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    const code = roomCode();
    const state = createOnlineState(color, targetPlayers, targetRounds);
...
function statusFor(error) {
  ...
  if (
    code === 'not_your_turn' || code === 'room_full' || code === 'version_conflict' ||
    code === 'color_taken' || code === 'room_not_playing' || code === 'round_not_finished' ||
    code === 'occupied_slot' || code === 'no_piece_remaining'
  ) return 409;
  if (
    code === 'invalid_color' || code === 'invalid_player_count' || code === 'invalid_round_count' ||
    code === 'invalid_move' || code === 'invalid_room_code' || code === 'invalid_payload' ||
    code === 'invalid_action'
  ) return 400;
  return 500;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (status >= 500) console.error('[Yakolak] v118 online room request failed', error);
    json(res, status, {
      ok: false,
