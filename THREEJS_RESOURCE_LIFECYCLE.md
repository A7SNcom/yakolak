...
15. Shared factories must return a concrete, newly unregistered resource. A factory may not smuggle an already active registry identity into a new shared key or silently convert an existing transient/generation registration into `shared-immutable`; ownership transfer/reclassification must be explicit outside the factory. If a newly created shared resource cannot be registered because the registry becomes unusable or registration otherwise fails, that unowned resource is cleaned up immediately with its explicit cleanup/default disposal path. A resource that was already actively owned before a rejected shared claim is never destroyed by this rollback.
...
19. Subscription adapters that synchronously notify a new subscriber during setup must be atomic. If that initial notification throws before the adapter can return its unsubscribe function, the adapter must remove the subscriber immediately and rethrow the original failure. A failed initial notification must never leave a live subscriber or a partial registry subscription token behind.
20. Reclassification updates only the core metadata explicitly supplied for the new ownership phase: `scope`, `ownership`, `kind`, `label` and `cleanup` (after any scope wrapper has deliberately injected its own scope/ownership). Omitted fields preserve the active registration instead of falling back to root/transient/inferred defaults. An explicit `cleanup` replaces the previous cleanup, including explicit `null` to return to kind-default cleanup. `replacementKey` keeps its separate stable-replacement semantics.
