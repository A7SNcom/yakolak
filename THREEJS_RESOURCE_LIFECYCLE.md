...
20. Reclassification transfers only metadata that is explicitly supplied for the new ownership phase. In particular, an explicit `cleanup` replaces the previously registered cleanup (including explicit `null` to return to kind-default cleanup), while an omitted `cleanup` preserves the prior cleanup. This keeps transient staging → shared cache promotion safe without silently retaining a stale cleanup when a new owner deliberately supplies one.
