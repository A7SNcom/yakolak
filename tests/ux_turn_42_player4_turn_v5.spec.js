...
async function transition(shared, pages, timeline, owner) {
  ...
  for (const { page, seat } of pages.filter(p => p.seat !== ownerSeat)) {
    ...
    const beforeMoves = shared.clients[seat].moves;
    ...
    expect(after.phase, `${label}/${seat}: remote interaction stays waiting`).toBe('waiting');
    expect(after.selectedSize, `${label}/${seat}: remote tap cannot mutate selection`).toBe(before.selectedSize);
    expect(after.tray, `${label}/${seat}: remote tap cannot mutate tray`).toBe(before.tray);
    expect(shared.clients[seat].moves, `${label}/${seat}: remote tap cannot send move`).toBe(beforeMoves);
    timeline.push({
      label: `${label}:remote-blocked`, seat,
      gameplayReady: after.gameplayReady,
      interactionPhase: after.phase,
      selectionUnchanged: after.selectedSize === before.selectedSize,
      trayUnchanged: after.tray === before.tray,
      transportMoveDelta: shared.clients[seat].moves - beforeMoves,
    });
  }
...
test('UX-TURN-42 P3→P4→P1 keeps one visible and interactive owner', async ({ browser }) => {
  ...
  try {
    ...
    console.log('UX_TURN_42_P3_P4_P1_PASS');
  } catch (error) {
