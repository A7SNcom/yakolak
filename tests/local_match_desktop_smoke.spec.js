...
async function playMove(page, size, cell, expectedMoves, expectedNextPlayer) {
  ...
  for (const coordinate of Object.values(points)) {
    expect(Number.isFinite(coordinate)).toBe(true);
  }
  expect(points.pieceX).toBeGreaterThan(0);
  expect(points.pieceX).toBeLessThan(1440);
  expect(points.pieceY).toBeGreaterThan(0);
  expect(points.pieceY).toBeLessThan(900);
  expect(points.cellX).toBeGreaterThan(0);
  expect(points.cellX).toBeLessThan(1440);
  expect(points.cellY).toBeGreaterThan(0);
  expect(points.cellY).toBeLessThan(900);

  await page.mouse.click(points.pieceX, points.pieceY);
  await page.waitForFunction(() => document.body.dataset.yakolakGameplay === 'piece-selected', null, { timeout: 15000 });
  await page.mouse.click(points.cellX, points.cellY);
...
test('desktop completes a full local round and starts the next round', async ({ page }) => {
  ...
  await page.screenshot({ path: 'web/local-match-desktop-round-win.png', fullPage: false, timeout: 60000 });

  await page.waitForFunction(
    () => document.body.dataset.yakolakRoundActionVisible === 'true' &&
      document.getElementById('yakolak-round-action')?.style.display === 'block',
    null,
    { timeout: 5000 }
  );
