...
# synthesize an early or repeated handoff.
...
func _run() -> void:
	...
	# smooth inactive. These values are no longer authority to start gameplay.
	...
	# but it still cannot hand control back until that new generation completes.
	intro.call("_restart_intro")
	...
	# must not reopen or consume another intro handoff generation.
...
func _finish() -> void:
	...
	if failures.is_empty():
		print("YAKOLAK_INTRO_HANDOFF_HEADLESS_OK emits=2 consumes=2 replay=one-shot rematch=stable")
		quit(0)
		return
