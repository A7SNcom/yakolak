...
const seats = [
  { seatId: 'right', type: 'human', color: 'marble', ready: true },
  { seatId: 'back', type: 'computer', color: 'blue', ready: true },
];
const setup = { preferredColor: 'marble', targetPlayers: 2, seats };
...
const accepted = {
  ok: true,
  code: null,
  placement: { seatId: 'right', color: 'marble', cell: 4, size: 'medium' },
};
...
for (const caller of ['human', 'bot', 'local-authority', 'backend-authority']) {
  assert.deepEqual(
    validatePlacementForSeat(state, 'right', { cell: 4, size: 'medium' }),
    accepted,
    `${caller} must receive identical placement legality`,
  );
}
assert(Object.isFrozen(validatePlacementForSeat(state, 'right', { cell: 4, size: 'medium' })));
...
assert.deepEqual(validatePlacementForSeat(state, 'front', { cell: 0, size: 'small' }), { ok: false, code: C.UNKNOWN_SEAT });

const occupiedBoard = emptyBoard();
occupiedBoard['4'].medium = 'blue';
const occupiedState = createCanonicalSessionState({ ...setup, board: occupiedBoard });
assert.equal(placementRejectionCode(occupiedState.board, 'marble', { cell: 4, size: 'medium' }), C.OCCUPIED_SLOT);
assert.deepEqual(validatePlacementForSeat(occupiedState, 'right', { cell: 4, size: 'medium' }), { ok: false, code: C.OCCUPIED_SLOT });

const exhaustedBoard = emptyBoard();
for (const cell of [0, 1, 2]) exhaustedBoard[String(cell)].small = 'marble';
const exhaustedState = createCanonicalSessionState({ ...setup, board: exhaustedBoard });
assert.deepEqual(exhaustedState.inventory.right, { small: 0, medium: 3, large: 3 });
assert.deepEqual(deriveRemainingInventoryFromState(exhaustedState).right, { small: 0, medium: 3, large: 3 });
assert.equal(placementRejectionCode(exhaustedState.board, 'marble', { cell: 3, size: 'small' }), C.NO_PIECE_REMAINING);
assert.deepEqual(validatePlacementForSeat(exhaustedState, 'right', { cell: 3, size: 'small' }), { ok: false, code: C.NO_PIECE_REMAINING });
...
fakeMorePieces.inventory.right.small = 3;
assert.deepEqual(
  validatePlacementForSeat(fakeMorePieces, 'right', { cell: 3, size: 'small' }),
...
fakeNoPieces.inventory.right.medium = 0;
assert.deepEqual(
  validatePlacementForSeat(fakeNoPieces, 'right', { cell: 4, size: 'medium' }),
  accepted,
...
console.log('THREEJS-046/048 placement/inventory contract: PASS');
