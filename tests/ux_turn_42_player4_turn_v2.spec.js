...
async function installClient(page, shared, seat) {
  ...
  await page.addInitScript(({ code, seat }) => {
    ...
    const sample = () => {
      ...
      if (!prev || JSON.stringify({ ...prev, t: 0 }) !== JSON.stringify({ ...row, t: 0 })) {
        ...
        if (window.__ux42Timeline.length > 120) window.__ux42Timeline.shift();
      }
  ...
  await page.route('**/api/rooms**', async route => {
    ...
    if (body.action === 'move') {
      ...
      return route.fulfill({ status: 409, contentType: 'application/json', body: JSON.stringify({ ok: false, error: seat === ownerSeat ? 'occupied_slot' : 'not_your_turn' }) });
    }
...
async function snap(page) {
  return page.evaluate(code => {
    ...
    return {
      ...
      samples: Array.isArray(window.__ux42Timeline) ? window.__ux42Timeline.slice(-40) : [],
    };
  }, CODE);
}
...
async function waitVisibleLivePiece(page, owner) {
  return expect.poll(async () => {
    const before = await snap(page);
    if (before.authPlayer !== owner || before.presentation !== 'transitioning') return null;
    const s = await refreshLiveTarget(page);
    const visible = Number.isFinite(s.authorityPieceX) && Number.isFinite(s.authorityPieceY) && s.authorityPieceX >= 2 && s.authorityPieceX <= VIEWPORT.width - 2 && s.authorityPieceY >= 2 && s.authorityPieceY <= VIEWPORT.height - 2;
    return visible ? s : null;
  }, { timeout: 10000, message: `P${owner}: live owner piece becomes tappable before presentation settles` }).not.toBeNull();
}
...
async function assertSettled(shared, pages, timeline, label, owner) {
  ...
  for (const { page, seat } of pages) {
    ...
    expect(s.seat, `${label}/${seat}: seat`).toBe(seat);
    ...
    expect(s.authDirection, `${label}/${seat}: owner direction`).toBe(direction);
    expect(s.hudVisible, `${label}/${seat}: visible indicator`).toBe('true');
    expect(s.hudPlayer, `${label}/${seat}: indicator owner`).toBe(owner);
    expect(s.hudText, `${label}/${seat}: indicator copy`).toBe(indicatorText(owner, seat));
    ...
    expect(s.hudRevision, `${label}/${seat}: same authority revision`).toBe(s.authRevision);
    expect(s.currentPlayer, `${label}/${seat}: gameplay owner`).toBe(direction);
    expect(s.turnRemaining, `${label}/${seat}: timer owner none`).toBe(0);
    ...
    if (seat !== ownerSeat) expect(ownerSamples.some(sample => sample.gameplay === 'ready'), `${label}/${seat}: remote never ready`).toBe(false);
    const row = { label, seat, roomVersion: client.lastVersion, turnIndex: client.lastTurnIndex, hydrated: s.hydrated, authRevision: s.authRevision, indicator: s.hudText, activeCue: s.hudColor, timerOwner: 'none', gameplayReady: s.gameplayReady, legalInputOwner: seat === ownerSeat, presentation: `${s.presentation}:${s.presentationTarget}:${s.presentationSettled}` };
    timeline.push(row);
    rows.push(row);
  }
  return rows;
...
async function transitionWithRealTap(shared, pages, timeline, label, owner) {
  const ownerSeat = `p${owner}`;
  ...
  const immediateTap = (async () => {
    await ownerPage.waitForFunction(ownerNumber => {
      const d = document.body.dataset;
      return Number(d.yakolakAuthoritativeTurnPlayer || 0) === ownerNumber && Number(d.yakolakTurnIndicatorPlayer || 0) === ownerNumber && d.yakolakTurnIndicatorLocal === 'true' && d.yakolakTurnPresentationState === 'transitioning';
    }, owner, { timeout: 12000 });
    await waitVisibleLivePiece(ownerPage, owner);
    const target = await refreshLiveTarget(ownerPage);
    expect(target.currentPlayer, `${label}: gameplay owner switches before settle`).toBe(DIRECTIONS[owner - 1]);
    expect(target.presentation, `${label}: first tap occurs during camera motion`).toBe('transitioning');
    ...
    timeline.push({ label: `${label}:first-real-tap`, seat: ownerSeat, firstTapAccepted: true, gameplayReadyAtTap: target.gameplayReady, presentationAtTap: `${target.presentation}:${target.presentationTarget}:${target.presentationSettled}`, selectedSize: accepted.selectedSize, tray: accepted.tray });
  })();

  await Promise.all(pages.map(({ page }) => wake(page)));
  await immediateTap;
  await Promise.all(pages.map(({ seat }) => expect.poll(() => shared.clients[seat].lastVersion, { timeout: 12000 }).toBe(shared.room.version)));
  await Promise.all(pages.map(({ page, seat }) => page.waitForFunction(({ owner, local }) => {
    const d = document.body.dataset;
    return Number(d.yakolakAuthoritativeTurnPlayer || 0) === owner && Number(d.yakolakTurnIndicatorPlayer || 0) === owner && d.yakolakTurnIndicatorLocal === (local ? 'true' : 'false');
  }, { owner, local: seat === ownerSeat }, { timeout: 12000 })));

  // Every remote client taps the same authoritative owner's live piece and must
  // stay inert. The owner already accepted its first tap while presentation moved.
  for (const { page, seat } of pages.filter(entry => entry.seat !== ownerSeat)) {
    const target = await refreshLiveTarget(page);
    if (Number.isFinite(target.authorityPieceX) && Number.isFinite(target.authorityPieceY) && target.authorityPieceX >= 2 && target.authorityPieceX <= VIEWPORT.width - 2 && target.authorityPieceY >= 2 && target.authorityPieceY <= VIEWPORT.height - 2) {
      await page.touchscreen.tap(target.authorityPieceX, target.authorityPieceY);
...
async function assertOwnerMoveTransport(shared, pages, timeline, label, owner) {
  ...
  for (const { page } of pages) {
    const t = await refreshLiveTarget(page);
    if (Number.isFinite(t.authorityCellX) && Number.isFinite(t.authorityCellY) && t.authorityCellX >= 2 && t.authorityCellX <= VIEWPORT.width - 2 && t.authorityCellY >= 2 && t.authorityCellY <= VIEWPORT.height - 2) {
      await page.touchscreen.tap(t.authorityCellX, t.authorityCellY);
    }
  }
  await expect.poll(() => shared.clients[ownerSeat].moves, { timeout: 4000, message: `${label}: owner move reaches transport` }).toBe(before[ownerSeat] + 1);
...
async function assertOnceOnly(pages, revisions, owner) {
  ...
  for (const { page, seat } of pages) {
    ...
    expect(s.authRevision, `P${owner}/${seat}: owner activates once`).toBe(revisions[seat]);
  }
}
...
test('UX-TURN-42 mobile P3→P4→P1 has one immediate real-tap owner', async ({ browser }) => {
  ...
  try {
    ...
    await transitionWithRealTap(shared, pages, timeline, 'P3→P4', 4);
    ...
    await transitionWithRealTap(shared, pages, timeline, 'P4→P1', 1);
