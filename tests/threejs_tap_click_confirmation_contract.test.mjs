...
const beforeFeedback = h.feedback.length;
...
assert.equal(h.feedback.length, beforeFeedback + 1, 'size feedback must happen before tapSize returns');
...
assert.equal(h.submissions.length, 1, 're-entrant submit must not duplicate authority mutation');
...
assert.match(source, /resolveBoardCellPick/);
