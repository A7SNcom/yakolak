...
async function openRestoredRoom(page, code) {
  ...
  await page.waitForFunction(() => document.body.dataset.yakolakIntroHandoffEvent === 'consumed', null, { timeout: 30000 });
  await page.waitForFunction(() => document.body.dataset.yakolakGameplay === 'ready' && document.body.dataset.yakolakAuthoritativeTurnValid === 'true' && document.body.dataset.yakolakTurnIndicatorVisible === 'true' && document.body.dataset.yakolakTurnIndicatorContract === 'pass', null, { timeout: 30000 });
}
...
test('UX-TURN-37 initial authoritative local turn is unmistakably local', async ({ page }) => {
  test.setTimeout(90000); mkdirSync('artifacts', { recursive: true });
