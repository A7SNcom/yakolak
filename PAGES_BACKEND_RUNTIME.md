...
GitHub Actions run `32019036397` proved the Worker contract tests and Wrangler v4 dry-run bundle pass. Deploy is intentionally manual.

The previous deployment attempt `32018877918` stopped at the credential gate before invoking Cloudflare because these four required environment values were empty:
...
That failure is authoritative evidence that no live Worker/Turso pair may currently be claimed.

## Live deployment identity

`.github/workflows/pages-005-cloudflare-backend.yml` now uses Wrangler's structured NDJSON output as the source of deployment identity rather than scraping console text or trusting an inferred hostname.
...
1. `wrangler deploy` creates and deploys the bootstrap version.
...
6. A browser running from the real GitHub Pages origin must pass the CORS health probe.
7. Only after all proofs succeed are `backend/cloudflare/API_ORIGIN.txt` and `backend/cloudflare/WORKER_ROLLBACK_WINDOW.json` committed.
...
Missing credentials, propagation delay, identity mismatch, CORS failure, Turso failure, or inability to prove both exact Worker versions prevents the lock files from being written. It does **not** block static presentation or local gameplay.
