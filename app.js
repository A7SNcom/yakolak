...
async function installVisualConnectionHotfix(){
  for(let i=0;i<420;i++){
    ...
    if(game?.render&&entry?.choose&&wall?.group&&services?.serviceGroup){
      ...
      // Keep every projection just in front of its wall instead of one unit behind it.
      ...
      // Route the actual menu choice to the room-native screens.
      ...
      // Absolute fallback: legacy buttons may still own an old onclick closure.
